...
TEST_CIRC_SEQ::TEST_CIRC_SEQ(unsigned number_bits, bool dbg, long l){
	bits = number_bits;
	debug = dbg;
	set_params(l);
	initialize();
...
void TEST_CIRC_SEQ::set_params(long l){
	params.p = 2;
	params.r = 1;
...
bool TEST_CIRC_SEQ::test_RCADDER(){
  ...
  he.RCADDER(k[0],k[1]);
    ...
    // std::cout << "results[" << i << "] = " << results[i]
      ...
      // << " , inputs[1][" << i << "] = " << inputs[1][i] << std::endl;
...
bool TEST_CIRC_SEQ::test_KSAdder(){
  make_copies();
