...
TEST_CIRC_COMB::TEST_CIRC_COMB(long l, bool dbg){
	debug = dbg;
	set_params(l);
	initialize();
}
...
void TEST_CIRC_COMB::t_start(){
  t_.start();
}

void TEST_CIRC_COMB::t_end(string name){
  double duration = t_.end("silent");
  std::cout << "--------------------------------------------" << std::endl;
  cout << className() << ": " << " m = " << params.m << std::endl;
...
void TEST_CIRC_COMB::set_params(long l){
	params.p = 2;
	params.r = 1;
	params.d = 0;
	params.k = 128;
