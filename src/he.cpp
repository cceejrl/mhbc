...
// Carry-look-ahead adder, Kogge-Stone
// see reference in https://en.wikipedia.org/wiki/Kogge%E2%80%93Stone_adder 
//Here assume the test bit width should be
//1,2,4,8,16,32,64
//others will be supported with a bit pad in the future
unsigned HE::depth(unsigned int bits){
  unsigned int res = 0;
  switch(bits){
    case 1:  res = 0; break;  
    case 2:  res = 1; break;
...
void HE::KS_RED(mkt k1, mkt k2){
  /*  Purpose: actually it's the leaf of Kogge-Stone adder
   *          Inputs:  k1 and k2                
   *          Outputs: o_p, o_g*/
  mkt o_p = copy(k1);
  XOR(k1, k2);
...
//interface may changenot using when bit width <=2
void HE::KS_GREEN(mkt k1, mkt k2, mkt o_p, mkt o_g){

}
//interface may change,not using when bit width <=2
void HE::KS_YELLOW(
        mkt p_in, mkt p_in_prev, mkt p_out,
        mkt g_in, mkt g_in_prev, mkt g_out){

}


//see reference https://en.wikipedia.org/wiki/Kogge%E2%80%93Stone_adder
//KSA_RED related to the RED node leaf
//KSA_YELLOW related to the yellow carry in operator
//KSA_GREEN related to green circle
void HE::KoggeStoneAdder(vector<mkt> &k1_bit, vector<mkt> k2_bit){
  unsigned nbits = PAD_BITS(k1_bit, k2_bit);
  unsigned int ks_depth = depth(nbits);
  vector<mkt> Gbit(nbits);
  for(unsigned i = 0; i < nbits; i++){
    Gbit[i] = copy(k2_bit[i]);
  }

  k1_bit.push_back(copy(k_zeros));
  Gbit.push_back(copy(k_zeros));
  nbits++;
  
  for(int i = 0; i != nbits; ++i){
    KS_RED(k1_bit[i],Gbit[i]);
  }

  //when nbits = 1, no other calc
  if(0 == ks_depth){  //single bit adder, depth = 1
    k1_bit[1] = copy(Gbit[0]);
  }
  
  if(1 == ks_depth){  //two bits width adder, depth = 1 

    mkt p0 = copy(k1_bit[0]);
    mkt p1 = copy(k1_bit[1]);
    mkt g0 = copy(Gbit[0]);
    mkt g1 = copy(Gbit[1]);

    //s0, do noting
    // XOR(k1_bit[0],k_zeros); //do nothing
    
    //s1, S1 = (A1 XOR B1) XOR ((A0 AND B0) OR (A0 XOR B0)  AND Cin)
    OR(g0, p0);
    AND(g0, k_zeros);
    XOR(k1_bit[1], g0);

    //s2,S2 = (A2 XOR B2) XOR (((A1 XOR B1) AND ((A0 AND B0) OR (A0 XOR B0) AND Cin)) OR (A1 AND B1))
    AND(p1, g0);
    OR(p1, g1);
    XOR(k1_bit[2],p1);
